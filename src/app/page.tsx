...
import { ArrowRight, Sparkles, Map, Globe, Compass } from "lucide-react";

export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#FDFCFB] text-[#1A1A1A] font-serif selection:bg-orange-100">
      {/* NAVIGATION - Logic remains the same, style is elevated */}
      <nav className="flex justify-between items-center px-8 py-6 max-w-7xl mx-auto">
        ...
        {/* UNTOUCHED LOGIC: Still points to your /plan route */}
        <Button asChild variant="outline" className="rounded-none border-black uppercase text-[10px] tracking-widest px-8 hover:bg-black hover:text-white transition-all">
      ...
      <section className="relative px-6 pt-24 pb-16 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-end gap-16 mb-24">
          ...
          >
            ...
            {/* UNTOUCHED LOGIC: Still points to your /plan route */}
            <Button asChild className="group h-16 px-10 bg-[#1A1A1A] text-white rounded-none hover:bg-orange-900 transition-all flex items-center gap-4 text-sm tracking-widest uppercase">
              <Link href="/plan">
                Start Your Story 
        ...
        {/* BENTO GRID (Using images that fit the brand) */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 h-auto md:h-[500px]">
          ...
          >
            ...
            <div className="absolute bottom-8 left-8 text-white">
              <span className="text-[10px] uppercase tracking-[0.3em] bg-white/20 backdrop-blur-md px-4 py-2 border border-white/30">Volume I</span>
          ...
          <div className="md:col-span-5 grid grid-rows-2 gap-6">
            <div className="bg-[#FAF7F2] p-10 flex flex-col justify-between border border-orange-100/50">
              <Sparkles className="text-orange-800 w-8 h-8 opacity-50" />
              <div>
                <h4 className="text-2xl font-medium mb-3 tracking-tight">AI with Soul</h4>
                <p className="text-gray-500 leading-relaxed text-sm italic">Our Claude-powered engine understands 'vibe' as much as 'coordinates'.</p>
              </div>
            </div>
            <div className="bg-white p-10 flex flex-col justify-between border border-gray-100 shadow-sm">
              <Compass className="text-gray-400 w-8 h-8 opacity-40" />
              <div>
                <h4 className="text-2xl font-medium mb-3 tracking-tight">Verified Truth</h4>
      ...
      <footer className="mt-32 py-12 text-center border-t border-gray-100">
        <p className="text-[10px] uppercase tracking-[0.4em] text-gray-400">TripCraft © 2026</p>
      </footer>
    </div>
  );
}